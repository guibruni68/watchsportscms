                    ...
                    if (agenteSelecionado && !field.value?.find(a => a.id === agenteId)) {
                      ...
                      field.onChange([...(field.value || []), novoAgente]);
                          ...
                          <SelectContent>
                            ...
                            {mockPlayers.filter(jogador => !field.value?.find(a => a.id === jogador.id)).map(jogador => <SelectItem key={`jogador-${jogador.id}`} value={jogador.id}>
                                {jogador.name} - #{jogador.number}
                              </SelectItem>)}
                            
                            {/* Times disponíveis */}
                            {mockTeams.filter(time => !field.value?.find(a => a.id === time.id)).map(time => <SelectItem key={`time-${time.id}`} value={time.id}>
                                {time.name}
                              </SelectItem>)}
                          </SelectContent>
                        </Select>
                      ...
                      {field.value && field.value.length > 0 && <div className="border rounded-lg p-4 bg-muted/50">
                          <div className="flex items-center justify-between mb-3">
                            <p className="text-sm font-medium">Agentes Selecionados ({field.value.length})</p>
                            <Button type="button" variant="ghost" size="sm" onClick={() => field.onChange([])} className="text-destructive hover:text-destructive">
                              Limpar todos
                            </Button>
                          </div>
                          <div className="space-y-2">
                            {field.value.map((agente, index) => <div key={`${agente.id}-${index}`} className="flex items-center justify-between p-2 bg-background border rounded-md">
